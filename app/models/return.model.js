...
async function addReturn(returnObj) {
    try{
            const result = await connection.query(`
            INSERT INTO returns (product_id, return_type, quantity, return_date, order_id,issue_id)
            VALUES ($1, $2, $3, $4, $5,$6) RETURNING *`, [returnObj.product_id, returnObj.return_type, returnObj.quantity, returnObj.return_date, returnObj.order_id,returnObj.issue_id]);
...
async function updateReturn(id, returnObj) {
    try{
        const result = await connection.query(`
            ...
            WHERE id=$7 RETURNING *`, [returnObj.product_id, returnObj.return_type, returnObj.quanity, returnObj.return_date, returnObj.order_id, returnObj.issue_id, id]);
        return result.rows[0];
    }catch(error){
        throw error;
    }
